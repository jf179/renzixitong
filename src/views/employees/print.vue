    ...
    <div class="app-container">
      <el-card>
        <el-breadcrumb separator="/" class="titInfo ">
          ...
          <el-breadcrumb-item>
            <span v-print="{ id: 'print' }">打印</span></el-breadcrumb-item
          >
        </el-breadcrumb>
        <div id="print">
          <!-- 个人详情 -->
          <div v-if="type === 't1' || type === 't2'">
...
打印插件
    ...
    2:导入   import PrintNb from 'vue-print-nb'
            Vue.use(PrintNb)
    3:按钮   v-print="{id:需要打印区域的id}"
...
import { employeesPersonalInfo, getJobDetail } from '@/api/employees'
export default {
  data () {
    return {
      formData: {},
  ...
  // 创建完毕状态
  created () {
    this.type !== 't3' ? this.getUserInfo() : this.getJob()
  ...
  methods: {
    async getUserInfo () {
      ...
      const res2 = await employeesPersonalInfo(this.userId)
      this.formData = { ...res, ...res2 }
    },
    async getJob () {
